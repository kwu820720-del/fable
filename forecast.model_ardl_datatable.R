#' Fast ARDL forecast using data.table, compatible with fable/tsibble (returns vector)
forecast.model_ardl <- function(object, new_data, specials = NULL, ...) {
  requireNamespace("data.table")
  requireNamespace("distributional")

  # Extract key/index info
  key_cols <- intersect(names(new_data), names(object$xreg))
  idx_col <- setdiff(names(new_data), c(key_cols, names(specials$xreg[[1]]$xreg)))
  ...
  start_row <- max(1, nrow(xreg_full) - nrow(new_data) + 1)
  xreg_lag <- xreg_full[start_row:nrow(xreg_full), ..needed_cols]
  ...
  beta <- unname(coef[(order[yvar] + 2):length(coef)])
  ...
  out[, .sim := as.numeric(ar_filter(yx, coef[2:(order[yvar] - lag_starts[[yvar]] + 2)], init_vals)), by = key_cols]
  ...
  # Return only the distribution column as a vector
  out$.dist
}